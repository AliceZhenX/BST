...
class MoviePrediction(data.Dataset):
  def __init__(self, dataset, user_id):
    ...
    user_id_df = dataset.iloc[dataset.groupby("user_id").groups[user_id]].copy() #选取当前的用户
    user_id_df["movie_ids"] = user_id_df["movie_ids"] .apply(lambda x: x[:-1]) #用户看过的电影，这里[:-1]？
    user_id_df["ratings"] = user_id_df["ratings"] .apply(lambda x: x[:-1]) #用户打分过的电影
    #这里不能改动，否则模型的参数不适用

    movies_watched_by_user = user_id_df["movie_ids"].values
    self.movies_watched_by_user = set(sum(movies_watched_by_user, []))
    #sum(movies_watched_by_user, []) 把用户看过的电影汇总起来
    ...
    dataset = user_id_df.explode(column = "target_movie_id") #target列表[1,3,5]拆成每个用户分别对应1 3 5多条数据
    #让同样的 序列数据 对应不同的 目标电影
    self.len = len(dataset)
    self.user_id = torch.tensor(dataset.user_id.values, dtype=int)
    self.gender = torch.tensor(dataset.gender.values, dtype=int)
    self.occupation = torch.tensor(dataset.occupation.values, dtype=int)
    ...
    #这样每个电影预测多次？所以在最后输出的时候，只输出电影的最大评分
  def __len__(self):
    return self.len
...
prediction = trainer.predict(model, torch.utils.data.DataLoader(predict_data,batch_size=128,shuffle=False, num_workers=0))
prediction = torch.cat(prediction , dim=0).numpy()

columns = [f"movie_{i}" for i in range(1, PARAMETERS["sequence_length"])] + ["movie_id", "movie_score"]
...
pred_movies = pd.DataFrame({
    ...
    "movie_score": (movie_groups.movie_score.mean()*100).astype(int), #这里把max()改为mean()
...
users_movies = pd.concat([pred_movies.head(10),users_movies], axis=1)
users_movies.columns = ["推荐电影", "预测评分", "用户已观看电影"]
print(users_movies)
print("pred_movies:")
print(pred_movies.head(10))
